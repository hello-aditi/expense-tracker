...
function GoalsItem({ refreshData, goalsList, remainingAmount }) {
  ...
  const getTransactions = async () => {
    try {
      ...
      const total = result.reduce((sum, transaction) => sum + transaction.amountAdded, 0);
      setTotalAdded(total);
  ...
  return (
    <div>
      <div className="mt-5">
        <div
          className={`relative w-[350px] h-[300px] transition-transform duration-500 
            transform-style-preserve-3d ${isFlipped ? "rotate-y-180" : ""}`}
          onClick={() => setIsFlipped(!isFlipped)}
        >
          {/* Front Side */}
          <Card
            className={`absolute w-full h-full flex flex-col p-4 shadow-lg rounded-2xl 
                border-2 border-fuchsia-800 hover:shadow-xl backface-hidden ${isFlipped ? "hidden" : ""}`}
          >
            <div>
              <div className="flex justify-between items-center">
                <Badge
                  className={`text-white px-3 py-1 rounded-md text-xs shadow-md ${goalsList.priority === "High" ? "bg-red-500" : goalsList.priority === "Medium" ? "bg-yellow-500" : "bg-green-500"}`}
                >
                  {goalsList.priority}
                </Badge>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This action cannot be undone. This will permanently delete your current goal record along with its savings.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
              ...
              <div className="text-center">
                <span className="text-3xl">{goalsList.icon}</span>
                <div className="mt-2">
                  <h3 className="text-xl font-semibold text-gray-800">{goalsList.name}</h3>
                  <p className="text-gray-600 text-base">₹{goalsList.amount}</p>
                </div>
              </div>
            </div>
            ...
            <div className="w-full bg-fuchsia-200 h-2 rounded-full mt-5 mb-5">
              <div
                className="bg-gradient-to-r from-fuchsia-500 to-fuchsia-700 h-2 rounded-full"
                style={{
                  width: `${(totalAdded / goalsList.amount) * 100}%`,
                }}
              ></div>
              <div className="flex items-center justify-between mb-5 mt-2">
                <h2 className="text-xs text-gray-500">₹{totalAdded} Added</h2>
                <h2 className="text-xs text-gray-500">₹{goalsList.amount - totalAdded} Remains</h2>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-col">
              <Button
                variant="outline"
                className={`w-full ${isGoalAchieved ? "bg-green-500 text-white" : "border-fuchsia-600 text-fuchsia-600 hover:bg-fuchsia-100"}`}
          ...
          {/* Back Side */}
          <Card
            className={`absolute w-full h-full flex flex-col p-4 shadow-lg rounded-2xl border border-fuchsia-200 hover:shadow-xl backface-hidden transform rotate-y-180 ${isFlipped ? "" : "hidden"}`}
          >
            <h3 className="text-lg font-semibold text-fuchsia-700">Transactions</h3>
            ...
            <div className="mt-4" onClick={(e) => e.stopPropagation()}>
              <input
                ...
                className="w-full p-2 border rounded-md border-fuchsia-300 focus:ring-2 focus:ring-fuchsia-500"
                max={remainingAmount}
              ...
              <Button
                ...
                className="w-full mt-2 bg-fuchsia-600 hover:bg-fuchsia-700 text-white"
                disabled={newAmount <= 0 || newAmount > remainingAmount || isLoading || isGoalAchieved}
              >
